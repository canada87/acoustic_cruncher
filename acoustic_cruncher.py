...
if test == 'mean':
    ...
    if divisore != 0:
        ...
        # #######################################################
        p = figure(title='frequenzy trace', x_axis_label='Hz', y_axis_label='')
        p.line(fft_x, (fft_medio - np.mean(fft_medio))*1000, legend='Trend', line_width=2)
        st.bokeh_chart(p, use_container_width=True)

        timei = st.slider('Select the time region', 0, len(tempo), 0)
        p = figure(title='time trace', x_axis_label='sec', y_axis_label='V', x_range=(0,0.1))
        p.line(tempo[timei]['time (s)'].to_numpy(), tempo[timei]['volt'].to_numpy(), legend='Trend', line_width=2)
        st.bokeh_chart(p, use_container_width=True)
        # #######################################################
